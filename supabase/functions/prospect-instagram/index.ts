...
async function handleSmartSearch(body: any) {
  ...
  if (!FIRECRAWL_API_KEY) {
    return jsonResponse({
      error:
        "Firecrawl no está configurado. Conecta Firecrawl en Settings → Connectors.",
    }, 400);
  }
  const LOVABLE_API_KEY = Deno.env.get("LOVABLE_API_KEY");
  if (!LOVABLE_API_KEY) {
    return jsonResponse({ error: "LOVABLE_API_KEY no configurada." }, 400);
  }

  const { searchQuery, interests, location, followerRange } = body;
  const followerLabels: Record<string, string> = {
    nano: "1K-10K seguidores",
    micro: "10K-50K seguidores",
    mid: "50K-200K seguidores",
    macro: "200K-1M seguidores",
    mega: "más de 1M seguidores",
  };
  const followerLabel = followerRange && followerRange !== "any"
    ? followerLabels[followerRange]
    : "";
  ...
  queries.push(
    [
      "mejores influencers instagram",
      interests ? `de ${interests}` : "",
      location,
    ].filter(Boolean).join(" "),
  );
  queries.push(
    ["instagram @", interests, location, "seguidores perfil"].filter(Boolean)
      .join(" "),
  );
  if (interests && location) {
    queries.push(
      `ranking influencers ${interests} ${location} 2025 2026 instagram`,
    );
  }
  ...
  await Promise.all(queries.map(async (q) => {
    try {
      ...
      if (res.ok) {
        ...
        allItems.push(...(data.data || data.results || []));
      }
    } catch {}
  }));

  const combinedContent = allItems.map((item: any) =>
    `URL: ${item.url || ""}\nTitle: ${item.title || ""}\nDesc: ${
      item.description || ""
    }`
  ).join("\n---\n").slice(0, 20000);
  const excludeUsernames = (body.excludeUsernames || []).map((u: string) =>
    u.toLowerCase().replace("@", "")
  );

  const aiPrompt =
    `Extrae perfiles de Instagram de estos resultados de búsqueda.
CRITERIOS: Nicho: ${interests || "cualquiera"}, Ubicación: ${
      location || "cualquiera"
    }, Seguidores: ${followerLabel || "cualquiera"}, Búsqueda: ${
      searchQuery || "ninguna"
    }
...
Máximo 30 perfiles. Solo JSON.`;
  ...
  try {
    const aiData = await callLovableChat(LOVABLE_API_KEY, {
      ...
      messages: [{
        role: "system",
        content: "Extractor de perfiles de Instagram. Solo JSON.",
      }, { role: "user", content: aiPrompt }],
      temperature: 0.3,
    });
    const profiles = parseJsonFromText<any[]>(
      getAssistantText(aiData) || "[]",
      "array",
    );
    const results = profiles.map((p: any) => ({
      username: (p.username || "").replace("@", ""),
      description: `${p.followers_estimate || "?"} • ${p.location || ""} • ${
        p.niche || ""
      } • ${p.match_reason || ""}`,
      url: `https://www.instagram.com/${(p.username || "").replace("@", "")}/`,
    }));
    return jsonResponse({ results });
  } catch {}
  return jsonResponse({
    results: [],
    error: "No se pudieron extraer perfiles.",
  });
}
...
async function handleProfileGather(body: any) {
  const FIRECRAWL_API_KEY = Deno.env.get("FIRECRAWL_API_KEY");
  if (!FIRECRAWL_API_KEY) {
    return jsonResponse({ error: "Firecrawl no está configurado." }, 400);
  }
  const cleanUsername = body.username.replace("@", "").trim();
  const searchQuery =
    `"${cleanUsername}" instagram seguidores influencer perfil`;
  const res = await fetch("https://api.firecrawl.dev/v1/search", {
    method: "POST",
    headers: {
      Authorization: `Bearer ${FIRECRAWL_API_KEY}`,
  ...
  const data = await res.json();
  if (!res.ok) {
    return jsonResponse({ error: `Error: ${data.error || "Unknown"}` }, 400);
  }
  const items = data.data || data.results || [];
  const combinedContent = items.map((item: any) =>
    `${item.title || ""}\n${item.description || ""}`
  ).join("\n\n");
  return jsonResponse({
    ...
    title: items[0]?.title || cleanUsername,
...
async function handleAnalyzeProfile(body: any) {
  ...
  const systemPrompt =
    `Analista de marketing de Instagram para "Silvio Costa Photography" (fotografía, vídeo, drones, tours virtuales 360° en España y Portugal).
...
Solo JSON.`;
  ...
  try {
    ...
    if (isLovableStatus(error, 402)) {
      return jsonResponse({ error: "Créditos agotados" }, 402);
    }
    throw new Error("Error analizando perfil");
  }
  const raw = getAssistantText(aiData) || "{}";
  let analysis;
  try {
    analysis = parseJsonFromText(raw, "object");
  } catch {
    analysis = {
      followers_estimate: "?",
      engagement_quality: "medio",
      brand_affinity: 50,
      interests: [],
      location_detected: "No detectada",
      account_quality: raw.slice(0, 200),
...
async function handleGenerateDM(body: any) {
  ...
  const analysisCtx = profileAnalysis
    ? `\nAnálisis: ${profileAnalysis.followers_estimate} seguidores, Engagement: ${profileAnalysis.engagement_quality}, Afinidad: ${profileAnalysis.brand_affinity}%, Intereses: ${
      profileAnalysis.interests?.join(", ")
    }, Ubicación: ${profileAnalysis.location_detected}`
    : "";

  const systemPrompt =
    `Experta en outreach de Instagram para "Silvio Costa Photography" — estudio profesional de fotografía, vídeo, dron y tours virtuales 360° con Matterport en España y Portugal.
...
REGLAS: Máx 200 palabras, menciona algo específico del perfil, presenta servicios relevantes, tono: ${
      messageStyle || "cercano y profesional"
    }.
...
Para cada versión incluye **Enfoque** y **Mensaje completo**.`;
  ...
  return jsonResponse({ dms: getAssistantText(aiData) || "" });
}

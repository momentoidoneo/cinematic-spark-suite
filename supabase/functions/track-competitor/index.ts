...
import { callLovableChat, getAssistantText } from "../_shared/lovableAi.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let targets: any[] = [];
    if (competitor_id) {
      const { data } = await supabase.from("competitors").select("*").eq(
        "id",
        competitor_id,
      ).single();
    ...
    for (const comp of targets) {
      try {
        ...
        if (hasChanges && LOVABLE_API_KEY && lastSnap?.markdown) {
          try {
            const aiJson = await callLovableChat(LOVABLE_API_KEY, {
              model: "google/gemini-2.5-flash",
              messages: [
                {
                  role: "system",
                  content:
                    "Eres un analista. Compara dos versiones de una página web y resume los cambios clave en español en 3-5 viñetas cortas y accionables.",
                },
                {
                  role: "user",
                  content: `ANTERIOR:\n${
            ...
            changesSummary = getAssistantText(aiJson) || null;
          } catch (e) {
        ...
        await supabase.from("competitor_snapshots").insert({
          ...
          links_count: (fcData.data?.links || fcData.links || []).length ||
            null,
